...
public class EncadeamentoInterior {
     ...
     */
    public void criaHash(String nomeArquivoHash) throws FileNotFoundException, IOException {
        RandomAccessFile tabelaHash = new RandomAccessFile(nomeArquivoHash,"rw");
        for (int i = 0; i < HASH_FILE_SIZE; i++) {
            Cliente cliente = new Cliente(-1,"          ",i,Cliente.LIBERADO);
        ...
        if (resultBusca.a != 1) {            
            ...
            if (resultBusca.end != -1) {
                ...
                if (overflow) { // nao ha nenhum espaço livre na tabela
                    tabelaHash.close();
                    return -1; // inserçao invalida: overflow
                }
